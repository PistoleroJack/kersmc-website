...
(async () => {
  ...
  try {
    ...
    // handig voor debug in console: __auth0Client.getIdTokenClaims().then(c=>console.log(c))
    window.__auth0Client = auth0;
  } catch (err) {
  ...
  // Als dit pad een uitzondering is: niets afdwingen, maar koppel logout
  if (isExceptionPath(pathname) || pathname === "/dashboard/callback.html") {
    attachLogout();
    return;
  }
